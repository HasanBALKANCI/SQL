...
-- each invoice, sorted first by CustomerId (in ascending order), and then by total dollar amount  (in descending order).
...
-- for each invoice, but this time sorted first by total dollar amount (in descending order), and then by CustomerId (in ascending order).
...
/* When i compare the results of these two queries above, i notice that 
first result query CustomerId in ASC then query total in DESC.*/
...
-- whose billing address is USA or Germany or Norway or Canada and invoice date is at any point in 2010, sorted from newest to oldest.


SELECT *
FROM invoices
WHERE BillingCountry IN ('USA', 'Germany', 'Norway', 'Canada')
AND InvoiceDate  BETWEEN '2010-01-01 00:00:00' AND '2010-12-31 00:00:00:00'
